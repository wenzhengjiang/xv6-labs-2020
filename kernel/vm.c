...
kvminithart()
{
  kvminithart_new(kernel_pagetable);
}

void
kvminithart_new(pagetable_t kpagetable)
{
...
{
  kvmmap_new(kernel_pagetable, va, pa, sz, perm);
}

void
kvmmap_new(pagetable_t kpagetable, uint64 va, uint64 pa, uint64 sz, int perm)
{
...
// Free a kernel page table without freeing the
